...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (welcomeMessage && userName) {
    welcomeMessage.textContent = `Welcome back, ${userName.split(' ')[0]}!`;
  }
    ...
    trackBtn.addEventListener('click', function() {
      ...
      setTimeout(() => {
        ...
        if (trackingResult) {
          ...
          document.getElementById('resultAppStatus').textContent = mockData.status;
...
function loadDashboardStats() {
  ...
  const processingApplicationsEl = document.getElementById('processingApplications');
  ...
  if (approvedApplicationsEl) approvedApplicationsEl.textContent = stats.approved;
  if (processingApplicationsEl) processingApplicationsEl.textContent = stats.processing;
}
...
function loadRecentApplications() {
  ...
  recentApplicationsEl.innerHTML = applications.map(app => `
    <div class="application-item">
      ...
      <div class="app-status">
        ...
        <button class="btn btn-sm" onclick="trackApplication('${app.id}')">Track</button>
      </div>
    </div>
  `).join('');
}
